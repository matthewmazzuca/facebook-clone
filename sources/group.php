...
function PageMain() {
	...
	if(isset($_GET['name']) && !$feed->group_data['id']) {
		header("Location: ".$CONF['url']."/index.php?a=welcome");
	}
	...
	if($_GET['r'] == 'edit' && $feed->group_member_data['permissions'] == 2) {
		...
		if($_GET['r'] == 'members') {
			...
			$TMPL['messages'] = $feed->searchFriendsGroup($_GET['friends']);
	...
	$TMPL['url'] = $CONF['url'];
	$TMPL['title'] = (isset($_GET['name']) ? 'Group - '.$feed->group_data['title'] : $LNG['title_group']).' - '.$settings['title'];
